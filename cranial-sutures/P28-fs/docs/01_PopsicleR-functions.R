# When using PopsicleR workflow, do not load package
# Transition to Seurat v5 has deprecated several components, which are replaced here
# Run this code before moving on to data loading
...
plotGene <- function(genelist, umi, dir){
  ...
  suppressWarnings({pdf(file.path(dir, paste0("01d_QC_Hist_Check.pdf")), useDingbats=FALSE)
    ...
    {
      if(gene %in% row.names(umi@assays$RNA@layers$counts)) {
        expr_gene <- paste0(gene, "_expressed")
        umi@meta.data[, expr_gene] <- ifelse(LayerData(object=umi, assay = "RNA",layer="counts")[gene,]>0, "TRUE", "FALSE")
  ...
  suppressWarnings({pdf(file.path(dir, paste0("01e_QC_Scatter_Check.pdf")), useDingbats=FALSE)
    ...
    for(gene in genelist)
    {
      if(gene %in% row.names(umi@assays$RNA@layers$counts)) {
        ...
        gp3<-FeatureScatter(umi, feature1="nFeature_RNA", feature2="percent_mt", pt.size=0.2, cells=Cells(umi)[umi@meta.data[, expr_gene]=="TRUE"], col="#00BFC4") +
          #ggplot2::ylim(0,100) +
          ggplot2::ggtitle(" ")+
          theme(text=element_text(size=10),axis.title=element_text(size=10)) + NoLegend()
        gp4<-FeatureScatter(umi, feature1="nFeature_RNA", feature2="percent_mt", pt.size=0.2, cells=Cells(umi)[umi@meta.data[, expr_gene]=="TRUE"], col="#00BFC4") +
          ggplot2::xlim(0,x.zoom.genes) +
          #ggplot2::ylim(0,100) +
          ggplot2::ggtitle(" ")+
          theme(text=element_text(size=10),axis.title=element_text(size=10))+ NoLegend()
        
        up1<-FeatureScatter(umi, feature1="nCount_RNA", feature2="percent_mt", pt.size=0.2,  cells=Cells(umi)[umi@meta.data[, expr_gene]=="TRUE"], col="#00BFC4") +
          ggplot2::ggtitle(" ") +
          guides(colour = guide_legend(paste0(as.character(gene),"+")))+
          theme(text=element_text(size=10),axis.title=element_text(size=10))+ NoLegend()
        up2<-FeatureScatter(umi, feature1="nCount_RNA", feature2="percent_mt", pt.size=0.2,  cells=Cells(umi)[umi@meta.data[, expr_gene]=="TRUE"], col="#00BFC4") +
        ...
        gu1<-FeatureScatter(umi, feature1="nFeature_RNA", feature2="nCount_RNA", pt.size=0.2, cells=Cells(umi)[umi@meta.data[, expr_gene]=="TRUE"], col="#00BFC4") +
          ggplot2::ggtitle(" ") +
          theme(text=element_text(size=10),axis.title=element_text(size=10))+ NoLegend()
...
algo_plot_features <- function(dir, data, type, plot_name, organism){
  ...
  if ("doublets" %in% Cells(data@meta.data)){
...
SR_plots <- function(db_name, annot_db, data, directory, cluster_res) { ### aggiungere BBpar per settare diversi core
  ...
  var.sc <- SingleR(test=data@assays$RNA@layers$data, ref=annot_db, labels=annot_db$label.main, method="single")
  ...
  if(is.null(cluster_res)) {
    ...
    var.cl <- SingleR(test=data@assays$RNA@layers$data, ref=annot_db, labels=annot_db$label.main, method="cluster", clusters=data@meta.data[[sel_cluster]])
    data[[cl_anal]] <- paste0(data[[]][[sel_cluster]], ":", var.cl$labels[match(data[[]][[sel_cluster]], Features(var.cl))])
...
PrePlots <- function(sample_name, input_data, genelist=NULL, percentage=0.1, gene_filter=200, cellranger=TRUE, organism=c("human","mouse"), out_folder=getwd()){
  ...
  suppressWarnings({pdf(paste0(QC_dir, "01a_QC_violin_plots.pdf"), width=24, useDingbats=FALSE)
    vln1 <- VLN(umi, 10, feats="nFeature_RNA", colours= "tomato", 0.01)+ NoLegend() + theme(axis.text.x=element_text(angle=0, hjust=0.5)) + ggplot2::xlab("")
    vln2 <- VLN(umi, 10, feats="nCount_RNA", colours= "tomato", 0.01)+ NoLegend() + theme(axis.text.x=element_text(angle=0, hjust=0.5)) + ggplot2::xlab("")
    vln3 <- VLN(umi, 10, feats="percent_mt", colours= "dodgerblue", 0.01)+ NoLegend() + theme(axis.text.x=element_text(angle=0, hjust=0.5)) + ggplot2::xlab("")
    vln4 <- VLN(umi, 10, feats="percent_ribo", colours= "yellow", 0.01)+ NoLegend() + theme(axis.text.x=element_text(angle=0, hjust=0.5)) + ggplot2::xlab("")
    vln5 <- VLN(umi, 10, feats="percent_disso", colours= "forestgreen", 0.01)+ NoLegend() + theme(axis.text.x=element_text(angle=0, hjust=0.5)) + ggplot2::xlab("")
...
CalculateDoublets <- function(UMI, method=c("scrublet","scDblFinder"), dbs_thr='none', dbs_rate=NULL, dbs_remove=TRUE, out_folder=getwd()){
  ...
  if(method=="scrublet"){
    ...
    if(dbs_thr == 'none'| !"scrublet_score" %in% Cells(UMI@meta.data)){
      doublets <- scrubDoublets(as.matrix(UMI@assays$RNA@layers$counts), directory=QC_dir, expected_doublet_rate=0.1)
...
Normalize <- function(UMI, variable_genes=2000, out_folder=getwd()){
  ...
  par(mfrow = c(2,1))
  hist(colSums(as.matrix(umi@assays$RNA@layers$counts)), breaks=100, main="Total expression before normalization", xlab="Sum of expression")
  hist(colSums(as.matrix(umi@assays$RNA@layers$data)), breaks=100, main="Total expression after normalization", xlab="Sum of expression")
  invisible(dev.off())
...
#' @param organism The organism on which to perform the analysis. Can be human or mouse.
...
ApplyRegression <- function(UMI, organism=c("human","mouse"), variables='none', explore_PC=FALSE,  out_folder=getwd()){
  ...
  if(all(variables!='none') & !all(variables %in% Cells(UMI@meta.data))) {stop(red("ERROR: Specified variables are not in metadata field"))}
  PP_dir <- file.path(out_folder,"02.PreProcessing")
  suppressWarnings(if (!file.exists(PP_dir)){dir.create(PP_dir, recursive=T)})
  
  if(!all(c("S.Score", "G2M.Score")%in%Cells(UMI@meta.data))){
    cat(bold(green("Calculating Cell Cycle Score \n")))
    if(organism == 'human') {
      cc.genes$s.genes <- intersect(cc.genes$s.genes, row.names(UMI@assays$RNA@layers$counts))
      cc.genes$g2m.genes <- intersect(cc.genes$g2m.genes, row.names(UMI@assays$RNA@layers$counts))
      UMI <- AddModuleScore(UMI, s.features=cc.genes$s.genes, g2m.features=cc.genes$g2m.genes, assay = "RNA", layer = "data",set.ident=T)
    } else if(organism == 'mouse'){
      s_genes = gorth(cc.genes.updated.2019$s.genes, source_organism = "hsapiens", target_organism = "mmusculus")$ortholog_name
      g2m_genes = gorth(cc.genes.updated.2019$g2m.genes, source_organism = "hsapiens", target_organism = "mmusculus")$ortholog_name
      UMI <- CellCycleScoring(object = UMI, s.features=s_genes, g2m.features=g2m_genes)
    } else {stop("organism must be human or mouse")}
  }#end if
  
  ### scaling and regression
  all.genes <- Features(UMI)
  if (all(variables == 'none')){
    ...
    UMI <- suppressMessages(RunPCA(UMI, features=UMI[["RNA"]]@meta.data$var.features, npcs=30, do.print=F, verbose=FALSE))
    UMI2 <- suppressMessages(RunTSNE(UMI, reduction = "pca",dims = 1:20))
    UMI2 <- suppressWarnings(RunUMAP(UMI2,reductino = "pca", dims = 1:20, verbose=FALSE))
    ...
    four_plots(UMI2, "pca")
    four_plots(UMI2, "tsne")
    four_plots(UMI2, "umap")
    invisible(dev.off())
    ...
    ### perform PCA on the scaled data.
    UMI <- RunPCA(UMI, features=UMI[["RNA"]]@meta.data$var.features, npcs=30, do.print=F, verbose=FALSE)
...
CalculateCluster <- function(UMI, dim_pca, organism=c("human","mouse"), marker.list='none', PCA=TRUE, cluster_res=0.8, out_folder=getwd()){
  ...
  if(organism == 'human') {
    ...
    if(marker.list == 'none'){
      marker.list <- list(T_cell=c("Cd3d", "Cd3e", "Tnfrsf4"),
                          CD4=c("Cd4"), T_ProB=c("Il7r"),
                          ...
                          Erythrocyte=c("Cd235a", "Gypa", "Hba-a1", "Hba-a2"),
                          Endothelial=c("Mcam", "Vcam1", "Vwf", "Pecam1", "Sele", "Cd93", "Nectin3", "Tek"),
                          Epithelial=c("Cdh1", "Cd326", "Epcam"),
                          Fibroblast=c("Vim", "Pdgfra", "Pdgfrb"),
                          Osteoblast=c("Sp7","Bglap","Phex"),
                          Myofibroblast=c("Col4a1", "Fn1", "Itgb5", "Postn"),
                          Tendon=c("Scx", "Mkx", "Col3a1", "Col12a1", "Lum"),
                          MSC=c("Axin2", "Gli1", "Prrx1"),
                          Osteocyte=c("Dmp1","Sost"),
                          Osteoclast=c("Acp5","Cd44"),
                          Chondrocyte=c("Col2a1","Acan"),
                          Odontoblast=c("Dspp"),
                          Ameloblast=c("Amelx","Enam"),
                          Myocyte=c("Dmd","Myh2"),
                          Neural=c("Satb2","Gfap"))
    } else {
      signature_dir <- getwd()
  ...
  for(res.i in cluster_res) {
    ...
    plot(x = data.tree, direction = "rightwards", main=paste0("phylogenetic tree of clusters @ res", res.i))
  }
  invisible(dev.off())
  ...
  if (length(cluster_res)==1) {
    ...
    FC_col <- Cells(umi.markers)[grep("avg_log", Cells(umi.markers))]
    ...
    for(cluster in unique(top.markers.2$cluster)) {
      ...
      vg1 <- VLN(UMI, feats=genes[1], point =FALSE) + ggplot2::geom_boxplot(width=0.1, outlier.shape=NA) + NoLegend() + ggplot2::xlab("")
      if (length(genes)==2) {
        vg2 <- VLN(UMI, feats=genes[2], point =FALSE) +
  ...
  FTP(UMI, Cluster_dir, "03h_", "umap", ftp_h, unlist(marker.list), "_marker_list")
  FTP(UMI, Cluster_dir, "03h_", "tsne", ftp_h, unlist(marker.list), "_marker_list")
  ...
  marker_list <- list()
  for (m in marker.list) {marker_list <- c(marker_list,list(m[m%in% Features(UMI)]))}
  ...
  ### dotplot of markers for each cluster
...
MakeAnnotation <- function(UMI, organism=c("human","mouse"), marker.list='none', thresh=20, cluster_res=NULL, out_folder=getwd()){
  ...
  if(!is.null(cluster_res)&!all(paste0("RNA_snn_res.",cluster_res)%in%Cells(UMI@meta.data))) {
  ...
  if(organism == 'human'){
    ...
    for(single_annot in annotations){
      ...
      print(DTP(UMI, markers_sub, single_annot))
      invisible(dev.off())
    }
    ### corrplot
    for(single_annot in annotations){
    ...
    matrice_norm <- as.matrix(LayerData(UMI))
   ...
   ### Plot TSNE and UMAP with scMCA annotation
    ...
    annotation_plot(Annot_dir, "04c_", UMI, "UMAP", "clean_labels", "CellPopulations_scMCA")
    annotation_plot(Annot_dir, "04c_", UMI, "TSNE", "clean_labels", "CellPopulations_scMCA")
    ...
    print(DTP(UMI, marker.list, "scMCA_simple"))
    invisible(dev.off())
