# Feb 14 2024: updated functions for Seurat v5
...
ApplyRegression <- function(UMI, organism=c("human","mouse"), variables='none', explore_PC=FALSE,  out_folder=getwd()){
  ...
  if(!all(c("S.Score", "G2M.Score")%in%Cells(UMI@meta.data))){
    ...
    if(organism == 'human') {
      ...
      m.s.genes <- intersect(m.s.genes, row.names(UMI@assays$RNA@layers$counts))
      m.g2m.genes <- intersect(m.g2m.genes, row.names(UMI@assays$RNA@layers$counts))
...
CalculateCluster <- function(UMI, dim_pca, organism=c("human","mouse"), marker.list='none', PCA=TRUE, cluster_res=0.8, out_folder=getwd()){
  ...
  for(res.i in cluster_res) {
    ...
    data.tree <- Tool(object = clus_time_tree, layer = "BuildClusterTree")
  ...
  if (length(cluster_res)==1) {
    ...
    print(DoHeatmap(UMI, features=top.markers.10$gene, layer="scale.data") + NoLegend())
    invisible(dev.off())
  }
